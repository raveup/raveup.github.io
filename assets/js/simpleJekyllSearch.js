(function($) {
    $.fn.simpleJekyllSearch = function(options) {
        var settings = $.extend({
            ...
            template            : '<article><figure><img src="/assets/covers/{category}/{image}.jpg" title="{title} – {item}"></figure><header><h2><a href="{url}" title="{title} – {item}">{title}</a></h2><h3>{item}</h3><p>{category} #{volume} ({support})</p></header>{excerpt}</article>',
